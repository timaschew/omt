			...
			while (enum2.hasMoreElements()) {
				...
				while (enum3.hasMoreElements()) {
					...
					if ((currNote <= 127) && (currNote >= 0)) {
						...
						int y = (int) (((10 - currNote / 12)
								* octavePixelheight + (ePos)) - noteOffset[currNote % 12]);
						int x = (int) (Math.round(aNote.getDuration()
								* beatWidth)); // 480 ppq note
						int xRV = (int) (Math.round(aNote.getRhythmValue()
								* beatWidth)); // 480 ppq note
						...
						offScreenGraphics.setColor(Color.getHSBColor(
								theColours[aNote.getPitch() % maxColours][0],
								theColours[aNote.getPitch() % maxColours][1],
								(float) (0.7 - (aNote.getPitch() * 0.004))));
						// draw note inside
						if (aNote.getPitchType() == Note.MIDI_PITCH) {
							offScreenGraphics.fillRect(oldX, y - noteHeight
									+ thinNote, x, noteHeight * 2 - 2
									* thinNote);
						} else { // draw frequency derrived note
							int heightOffset = 7;
							for (int j = oldX; j < oldX + x - 4; j += 4) {
								offScreenGraphics.drawLine(j, y - noteHeight
										+ heightOffset, j + 2, y - noteHeight
										+ heightOffset - 3);
								offScreenGraphics.drawLine(j + 2, y
										- noteHeight + heightOffset - 3, j + 4,
						...
						// draw note ouside
						offScreenGraphics.setColor(Color.getHSBColor(
								theColours[i % maxColours][0], theColours[i
										% maxColours][1], (float) (0.4)));
						offScreenGraphics.drawRect(oldX, y - noteHeight
								+ thinNote, xRV, noteHeight * 2 - 2 * thinNote);
				...
				// offScreenGraphics.setColor(Color.lightGray);
//				offScreenGraphics.setColor(Color.getHSBColor(theColours[i
//						% maxColours][0], theColours[i % maxColours][1],
//						(float) (0.9)));
//				offScreenGraphics.drawRect(rectLeft - 1, rectTop - noteHeight
//						- 1, rectRight + 1, rectBot - rectTop + noteHeight * 2
//						+ 2);
			}
