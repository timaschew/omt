...
public final class MidiParser implements JMC {
	...
	private static void sortEvents(Score score, Vector evtList, Vector phrVct,
			SMF smf, Part part) {
		...
		for (int i = 0; i < evtList.size(); i++) {
			...
			if (evt.getID() == 007) {
				...
				if (dynamic > 0) {
					noteOn(phrIndex, curNote, smf, i, currentLength, startTime,
							phrVct, midiChannel, pitch, dynamic, evtList);
				}
	...
	private static void noteOn(int phrIndex, Note[] curNote, SMF smf, int i,
		...
		for (int p = 0; p < phrVct.size(); p++) {
			...
			// Marcel Karras: raus damit
			// if(currentLength[p]<=(startTime+0.08)){
			if (currentLength[p] <= (startTime)) {
				phrIndex = p;
				break;
			}
		}
		...
		if ((startTime > currentLength[phrIndex])
			...
			// Marcel Karras: raus damit
			/*
			 * if(newTime < 0.25){ double length=
			 * curNote[phrIndex].getRhythmValue();
			 * curNote[phrIndex].setRhythmValue( length+newTime); }else{
			 */
	...
	public static void scoreToSMF(Score score, SMF smf) {
		...
		smfT.addEvent(new TimeSig(0, score.getNumerator(), score
				.getDenominator()));
		...
		while (aEnum.hasMoreElements()) {
			...
			/*
			 * if(inst.getTempo() != Part.DEFAULT_TEMPO){
			 * //System.out.println("Adding part tempo"); midiEvents.add(new
			 * EventPair(0, new TempoEvent(inst.getTempo()))); }
			 */
			...
			if (inst.getInstrument() != NO_INSTRUMENT) {
				...
				// inst.getInstrument());
				midiEvents.add(new EventPair(0, new PChange((short) inst
						.getInstrument(), (short) inst.getChannel(), 0)));
			}

			if (inst.getNumerator() != NO_NUMERATOR) {
				midiEvents.add(new EventPair(0, new TimeSig(
						inst.getNumerator(), inst.getDenominator())));
			}

			if (inst.getKeySignature() != NO_KEY_SIGNATURE) {
				midiEvents.add(new EventPair(0, new KeySig(inst
						.getKeySignature(), inst.getKeyQuality())));
			}
			...
			while (partEnum.hasMoreElements()) {
				...
				if (phrase.getInstrument() != NO_INSTRUMENT) {
					midiEvents.add(new EventPair(0, new PChange((short) phrase
							.getInstrument(), (short) inst.getChannel(), 0)));
				}
				if (phrase.getTempo() != Phrase.DEFAULT_TEMPO) {
					phraseTempoMultiplier = scoreTempo / phrase.getTempo(); // (scoreTempo
																			// *
																			// partTempoMultiplier)
																			// /
																			// phrase.getTempo();
				...
				while (phraseEnum.hasMoreElements()) {
					...
					if (note.getPitchType() == Note.FREQUENCY) {
						System.err
								.println("jMusic warning: converting note frequency to the closest MIDI pitch for SMF.");
						// System.exit(1);
						pitch = Note.freqToMidiPitch(note.getFrequency());
					} else
						pitch = note.getPitch();
					if (pitch != REST) {
						midiEvents.add(new EventPair(new Double(startTime
								+ offsetValue), new NoteOn((short) pitch,
								(short) note.getDynamic(), (short) inst
										.getChannel(), 0)));
						...
						double endTime = startTime
								+ (note.getDuration() * phraseTempoMultiplier);
						// Add the note-off time to the list
						midiEvents.add(new EventPair(new Double(endTime
								+ offsetValue), new NoteOn((short) pitch,
								(short) 0, (short) inst.getChannel(), 0)));
					}
					// move the note-on time forward by the rhythmic value
					startTime += tickRounder(note.getRhythmValue()
							* phraseTempoMultiplier); // time between start
														// times
